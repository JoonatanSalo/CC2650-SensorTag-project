...
void mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
	...
	// JTKJ: 2. Muunnetaan 8-bittiset rekisterinarvot 16-bittisiksi. T�ss� siis 12
	//          8-bittist� rekisteriarvoa (kiihtyvyyden x,y,z ja gyron x,y,z joista
	//          kaikista sek� MSB ett� LSB. Taulukossa 16-bttinen data[0] koostuu
	//          siis rekisterien ACCEL_XOUT_H (=MSB) ja ACCEL_XOUT_L (=LSB)
	//          yhdistetyist� arvoista. data[1] on sitten vastaavat y-akselin arvot, jne
	...
	*az = (float)data[2]*aRes - accelBias[2];
	...
	// JTKJ: 4. Muunnetaan g-arvot kulmanopeudeksi. T�m�n muunnoksen voi tehd�
	//          halutesaan, kumpikin tapa datan esitykselle k�y
